...
def generate_apply_stream(req: GenerateAndApplyRequest, db=Depends(get_db)):
    ...
    def event_stream():
        applied = skipped = 0
        # TASK-046 — outer compute span for the whole SSE stream so
...
def color_tracks_stream_ep(req: ColorTracksRequest, db=Depends(get_db)):
    ...
    def event_stream():
        ...
        # TASK-046 — outer compute span for the SSE stream.
...
async def library_health(
    ...
    def event_stream():
        from .. import perf as _perf
        # TASK-046 — outer compute span for the library-health SSE stream.
...
def cue_tools_stream(req: CueToolsRequest, db=Depends(get_db)):
    ...
    def event_stream():
        ...
        # TASK-046 — outer compute span for cue-tools SSE.
        ...
        try:
            for i, tid in enumerate(track_ids):
                ...
                if content is None:
                    ...
                    try:
                        # TASK-046 — per-track write span (mutation block).
...
async def classify_library(
    ...
    def event_stream():
        from .. import perf as _perf
        # TASK-046 — outer compute span for classify SSE.
...
def auto_tag_discogs(req: DiscogsTagRequest, db=Depends(get_db)):
    ...
    def event_stream():
        ...
        from .. import perf as _perf
        # TASK-046 — outer compute span for auto-tag/discogs SSE.
...
async def enrich_comments_stream(req: EnrichCommentsRequest, db=Depends(get_db)):
    ...
    def event_stream():
        # TASK-046 — outer compute span for enrich-comments SSE.
            ...
            for fut in _as_completed(futures):
                ...
                if isinstance(payload, Exception):
                    ...
                    if req.dry_run:
                        ...
                        try:
                            # TASK-046 — per-track writer span (parallel).
                ...
                try:
                    ...
                    if content is None:
                        ...
                        if result is None:
                            ...
                            if not req.dry_run:
                                try:
                                    # TASK-046 — per-track writer span (serial).
                                    with _perf.perf_span("enrich_comments.write_one"):
                                        db.session.commit()
                                    undo_rows.append({"content_id": str(tid), "previous": previous})
                                except Exception as commit_exc:
