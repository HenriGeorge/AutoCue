...
Transition scoring is a pure function over two `DjmdContent` rows and a `Rekordbox6Database` handle. It is invoked from two surfaces:
...
`end_energy_a` and `start_energy_b` are **scalars**, not curves. They are deliberately exposed (as opposed to keeping the full curves internal) so that downstream consumers (set builder energy-penalty, transition advice) can compute energy deltas without re-reading ANLZ. See section 12 ("Performance").
...
`_key_score(key_a, key_b)` at `autocue/analysis/transitions.py:70` returns a value in [0, 100] based on the Camelot wheel.

### What Camelot is

The Camelot wheel arranges musical keys around a 12-position circle. Each position has two keys: `<n>A` (minor) and `<n>B` (major), where stepping from `nA` to `nB` is a relative major/minor swap on the same root. Adjacent positions on the wheel share the most pitches and produce the cleanest harmonic mixes.
...
Energy curves are 50-point normalized 0–1 arrays produced by `autocue/analysis/energy.py:get_energy_curve` from the PWAV section of the `.DAT` ANLZ file. The transition scorer averages a small window at each track edge:
