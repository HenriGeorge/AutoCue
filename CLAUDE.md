...
- 🚨 **`Kind=0` is SHARED by memory CUES and memory LOOPS** — the discriminator is `OutMsec` (`-1` = point cue, `> InMsec` = loop). So **never blanket-`DELETE` and never blanket-`COUNT` `Kind=0`**: a delete meant for cues destroys the DJ's hand-placed loops (and vice-versa), and a count that conflates them silently skips the write. Both halves go through the shared `_point_cue_filter()` (`db_writer.py:138`); loop writes are **append-only** (`write_loops_to_db`, no `overwrite`, no `DELETE`). Full invariant → `.claude/project/db-constraints.md`.
...
- **Library Duplicates** (`autocue/analysis/duplicates.py` + `GET /api/duplicates` SSE + `POST /api/duplicates/delete` SSE) groups tracks by case-insensitive whitespace-normalised **`(artist, title, duration_bucket)`** where `duration_bucket = round(duration/5)` (5 s buckets keep encoder-rounded re-imports together but split an album cut from an extended mix). The keeper is picked by `(existing_hot_cues, play_count, last_played, bitrate, -track_id)` — **cue-prep outranks plays** so a freshly-prepped re-import beats a heavily-played auto-cued original. The user can override the keeper per-group via the **"Keep" radio**; the delete-button label, non-keeper set, and same-path chips recompute live on flip. **`delete_tracks` cascades through all 13 ContentID-bearing tables** (verified by `tests/test_duplicates_integration.py` against a real in-memory SQLite — DjmdCue/ContentCue/ContentActiveCensor/ContentFile/DjmdActiveCensor/DjmdSongHotCueBanklist/DjmdMixerParam/DjmdSongSampler/DjmdSongHistory/DjmdSongPlaylist/DjmdSongTagList/DjmdSongMyTag/DjmdSongRelatedTracks before DjmdContent) — a schema-pinned test fails if pyrekordbox adds a new ContentID table. Safety: 409 when Rekordbox running; **per-session backup window** (deletes <30 s apart reuse one backup, so /api/restore rolls back the whole session); **per-row savepoint**; **concurrency lock** 409s a second concurrent real delete (shared-session guard, released on every SSE exit path incl. client disconnect). The SSE delete streams per-batch progress and honours client-disconnect as cancel (rows committed before cancel survive; backup still restores). UI: confirm modal with primary disabled 250 ms after open, Cancel focused, focus-trap, ESC-during-delete aborts; in-place progress bar; **inline "Undo this delete"** banner (30 s) that POSTs `/api/restore`; same-file vs distinct-file chip per copy + a modal audio-summary line. Frontend invalidation via `_onTracksDeleted(ids)` prunes parsedTracks/parsedTracksById/healthData surgically (no /api/tracks refetch) and now also `AppState.signal('tracks')` so rail crate counts + the status sentence repaint. **In the 2.0 workbench (P3) Duplicates is a rail *place***: the `#wb-dupes-place` rail entry swaps the centre pane to `#wb-dupes-pane` (toggle `hidden` + `body.wb-place-dupes` ONLY — `#track-list` never detached, TASK-033/037), lazy-scans on first open, and re-drives the legacy machinery via `window.ACBridge` (`scanDuplicates`/`openDuplicatesConfirm`/`onTracksDeleted`) — `docs/js/v2/workbench/duplicates.js` never fetches `/api/duplicates*` itself. The bulk-delete verb is the static `#wb-dupes-bulk-delete` toolbar pill; restore is **canonically** the A-layer status-sentence sheet (`#status-restore` fact → `#wb-restore-sheet`, `docs/js/v2/restore-sheet.js`, fed by the `autocue:duplicates-deleted` event), with the inline undo banner kept as the in-view convenience. The H consent gradient is NOT applied to duplicates-delete (faef4c4 left it out deliberately). Tests: `tests/test_duplicates.py`, `tests/test_duplicates_integration.py`, `TestDuplicatesEndpoint` + `TestDuplicatesDeleteEndpoint`, `tests/web/duplicates-*.test.js`, `tests/web/v2-duplicates-place.test.js`, `tests/e2e/v2-duplicates-place.spec.ts`.
- **Discover v2** lives in `autocue/analysis/discover/` (taste, style_graph, feeders/, ranker, scan_orchestrator, store) + REST surface under `/api/discover/*`. Per-scan budget is locked at **artist=20, label=15, novelty=10** (HARD_SCAN_REQUEST_CAP=60); changes need PRD §4 sync. Snooze durations are **1w / 1m / 3m** only — `'30d'` 400s the backend. `ScanConfig.timeout_seconds` (default **120s**) is a wallclock soft-cap that closes the scan row with `status='timeout'` if a feeder wedges — without it, a stuck urlopen pinned the concurrent-scan lock until restart (issue #174). The scan-supersede path in `runScan()` polls `/api/discover/feed/status` for ≤3s before issuing a new fetch so rapid filter changes don't race the orchestrator lock (issue #169). The filter UI is two rows: server-side (Source/Sort/Year) + client-side (search/style chips/hide-saved/hide-dismissed) persisted under `localStorage.ac_discover_filters`. Year options: `this`/`last2`/`last5`/`custom` (custom reveals an inline year input). **Discover is a workbench rail place** (P5, `docs/js/v2/workbench/discover.js` → `#wb-disc-place`): it swaps the centre pane to the restyled feed via `switchTab('discover')` (mandatory — the `_handleDiscoverKeydown` map gates on `#disc-v2-section.offsetParent`), re-hosts release detail in `#wb-inspector` (mode flag), and **re-drives the frozen `DiscoverV2` IIFE via `window.DiscoverV2` + `ACBridge.discover*` — never re-implements an `/api/discover/*` fetch**. Discover/Duplicates places are mutually exclusive. The legacy tab nav is **fully retired**: `#tab-discover` removed (P5); the `#tab-cues`/`#tab-library` buttons (`#tab-group`) are CSS-hidden (Library is now the `#wb-library-place` rail place). `switchTab` + the `#*-tab-content` blocks survive (the places use them).
- **Library place** (`docs/js/v2/workbench/library.js` → `#wb-library-place`) is a workbench rail place (like Duplicates/Discover) that swaps the centre pane to the Library tools surface (`#library-tab-content`: health, cue tools, discogs, comments, playlist-suggest, set-builder) via `switchTab('library')`. It **retired the Cues/Library tab bar**: `#tab-group` (the buttons) is CSS-hidden, `#tab-nav` stays only as the `#app-status` status-sentence row, Cues is the default centre. Mutually exclusive with Duplicates/Discover — 3-way cross-deactivate in each `activate()` + the shell/rail crate/playlist/saved-filter exit paths + the `placeActive` check (`shell.js`). Re-driven by ⌘K `go-library`/`health-scan` + the status-sentence health fact (all click `#wb-library-place`); `go-cues`/status-count deactivate the active place. The legacy `#tab-cues`/`#tab-library` markup + `switchTab` **REMAIN** (load-bearing; inventoried `skipSweep` since still in the DOM) — don't delete them. NO new analysis/endpoint — the tools are the unchanged legacy surfaces. Tests: `tests/e2e/v2-library-place.spec.ts`.
- **Nightboard** (P4, `docs/js/v2/nightboard/{mode,set-model,canvas,joint-popover,tray}.js` → `#nb-canvas`) is a full-bleed set-builder canvas **mode** the workbench swaps into (NOT a centre-pane place): `body.nb-active` hides rail + grid + inspector (grid HIDDEN not detached — TASK-033/037) and the canvas owns the body, keeping the global topbar. Entered by the `#nb-open-btn` toolbar verb (relocated via the shell `_TOOL_IDS`) + the `open-nightboard`/`build-set` ⌘K commands; local-mode only. **No new analysis, no new backend endpoint, no LLM** — it renders entirely off the existing `POST /api/setbuilder`, `POST /api/transitions/score`, `GET /api/setbuilder/alternatives`, `GET /api/tracks/{id}/energy`, `POST /api/playlists`. Initial joint scores reuse `SetBuilderTrackItem.transition_score` (zero round-trips); `/api/transitions/score` is called lazily on **popover-open** (for the 3 `explanation` strings) and on **swap/insert** (re-scores ONLY the ≤2 touched joints — `set-model.rescoreJoints`, never a full rebuild). Joint band cutoffs ≥85/≥70 are presentation constants in `canvas.js` (`JOINT_BANDS`). Net-new --zone-warmup/-build/-peak/-closing tokens (light+dark) in app.css (runtime source of truth), mirrored byte-equal into docs/design/tokens/colors.css. Tile-focus re-hosts the **existing P2 inspector** (`workbench/inspector.js`, mode 'track') via `body.nb-inspecting`; export delegates to the shared `_createPlaylist` write path (`ACBridge.createSetPlaylist`). Per-track duration/cues read the **camelCase** parsed fields (`totalTime`/`existingHotCues`) from `ACBridge.tracks()`, not the snake_case API. The legacy vertical `#setbuilder-section` stays reachable (parity-retired later). Tests: `tests/test_nightboard_contract.py` (schema-pin), `tests/web/nightboard-{set-model,canvas,popover,tray,interop}.test.js`, `tests/e2e/v2-nightboard.spec.ts`.
