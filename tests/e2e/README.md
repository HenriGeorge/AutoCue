...
(the `0-` prefix guarantees alphabetical ordering puts it ahead of every
other spec) and verifies — via `/api/status` + `X-AutoCue-Diagnostic: 1` header —
...
| Spec | Purpose |
|------|---------|
| `0-safety.spec.ts` | Verifies server is bound to sandbox DB; aborts run otherwise. `0-` prefix forces alphabetical-first discovery. |
