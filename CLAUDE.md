...
pytest                               # run all 1422 Python tests
npm install                          # one-time: install JS test deps
npm test                             # run 626 Vitest tests for the web app
...
- **Library Duplicates** (`autocue/analysis/duplicates.py` + `GET /api/duplicates` SSE) is **read-only** in phase 1 — groups tracks by case-insensitive whitespace-normalised `(artist, title)`, picks a keeper via `(play_count, existing_hot_cues, last_played, -track_id)` and surfaces them in the Library tab. No track deletes; the destructive path will land in a phase 2 PR with backup + `rekordbox_is_running()` guards. Tests at `tests/test_duplicates.py` (18 unit + 4 route specs).
