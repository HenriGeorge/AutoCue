...
- 🚨 **`Kind=0` is SHARED by memory CUES and memory LOOPS** — the discriminator is `OutMsec` (`-1` = point cue, `> InMsec` = loop). So **never blanket-`DELETE` and never blanket-`COUNT` `Kind=0`**: a delete meant for cues destroys the DJ's hand-placed loops (and vice-versa), and a count that conflates them silently skips the write. Loop writes must be **append-only**. Full invariant + the `write_cues_to_db`/`write_memory_loops` `overwrite=True` footgun → `.claude/project/db-constraints.md`.
