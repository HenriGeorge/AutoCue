...
// `buildIdSelector` lives in `./per-control-sweep.helpers` so the
// selector unit tests (`per-control-sweep.selector.test.ts`) can import it
// without one Playwright test file importing another — Playwright aborts
// test discovery when it sees that (issue #112). The implementation,
// rationale, and escaping rules are documented at the helper's definition.
