...
3. **Invoke the Workflow** (passes the args through):
   - `/autocue-fixer <num>` → `Workflow({ scriptPath: ".claude/workflows/autocue-fixer.js", args: { issues: [num], dryRun: false } })`
...
Full documentation with mermaid diagrams: `docs/qa_fixer.md`. Agent prompt: `.claude/agents/autocue-fixer.md`. Workflow script: `.claude/workflows/autocue-fixer.js`. Config: `.claude/fixer.yaml`.
