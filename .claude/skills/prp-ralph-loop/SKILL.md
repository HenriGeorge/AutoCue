...
2. **Test-first for any core logic or invariant** (see `prp-core:prp-tdd`): write the
   failing test, watch it fail, then implement minimally
...
### When a Validation Fails: Debug Systematically

For anything beyond a trivial fix (obvious typo, missing import), follow
`prp-core:prp-systematic-debug`:

1. **Root cause first** — read the full error, reproduce, check recent changes. NO fixes
   without root-cause investigation.
2. **One hypothesis, minimal change** — never stack multiple speculative fixes in one
   iteration; you can't isolate what worked.
3. **Failing test before the fix** — reproduce the bug in a test, then fix
   (`prp-core:prp-tdd`).
4. **3-strike rule** — if 3 fix attempts for the same failure have not worked, STOP. This
   is an architecture problem, not a fix problem. Log it in the progress log, do NOT
   output the completion signal, and end the iteration stating exactly what decision is
   needed from the user. Do not burn remaining iterations on attempt #4.

Track fix attempts per failure in the progress log (`Fix attempts for X: 2`) so the next
iteration knows the count.
...
### The Verification Gate (required before the signal)

Per `prp-core:prp-verify`: **no completion claims without fresh verification evidence.**
Before outputting the signal, in THIS iteration (not a previous one):

1. Re-run every validation command, fresh and complete
2. Read the full output — exit codes, failure counts
3. Re-read the plan and check every requirement line-by-line against what exists
4. Check `git status` — work committed, tree clean

"Passed last iteration", "should pass", or a partial run does NOT satisfy the gate.
...
All six verified with fresh output from this iteration — the verification gate above.
...
### Mistake 7: Thrashing on a Failing Validation
**Wrong:** Try fix after fix on the same failure, iteration after iteration
**Right:** Root cause first (`prp-core:prp-systematic-debug`). After 3 failed fix
attempts, stop and escalate — it's an architecture problem, not a fix problem

### Mistake 8: Code Before Test
**Wrong:** Implement the feature, then add tests that pass immediately
**Right:** Failing test first for core logic and every PRP invariant (`prp-core:prp-tdd`)
...
- [ ] Write failing test first for core logic (`prp-core:prp-tdd`)
- [ ] Implement next task
- [ ] Run type-check
- [ ] Run lint
- [ ] Run tests
- [ ] Run build (if applicable)
- [ ] On failure: root cause before fix; track attempt count (`prp-core:prp-systematic-debug`)
- [ ] Commit changes
- [ ] Update progress log
- [ ] Decide: complete or continue — completion requires the fresh-evidence
      verification gate (`prp-core:prp-verify`)
