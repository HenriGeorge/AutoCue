...
- 🚨 **`Kind=0` is SHARED by memory CUES and memory LOOPS — the discriminator is `OutMsec`** (`-1` sentinel = point cue; `> InMsec` = loop region). This is the same test `read_hot_cues` uses (`db_writer.py:174`) to decide whether a row is a loop. Two ways to get it catastrophically wrong — **both are handled in the current code; do not reintroduce either**:
  - **NEVER blanket-DELETE `Kind=0`.** A `.filter(DjmdCue.Kind == 0).delete()` issued to rewrite memory *cues* also destroys the DJ's hand-placed memory **LOOPS** (and one issued to rewrite loops would destroy their memory **CUES**). ⇒ `write_cues_to_db`'s memory rewrite (`db_writer.py:685`) deletes **point cues only**, via the shared predicate `_point_cue_filter()` (`db_writer.py:138`) = `OutMsec IS NULL OR OutMsec <= InMsec`. Loops are spared.
  - **NEVER blanket-COUNT `Kind=0` to gate a write.** Counting loops as memory cues means a track that has loops can never get a memory cue written — the write is **silently skipped**. ⇒ `has_existing_memory_cues()` (`db_writer.py:154`) counts **point cues only**, using the same `_point_cue_filter()`. The COUNT and the DELETE deliberately share one predicate so the two halves cannot drift apart.
  - **A loop write is append-only.** `write_loops_to_db()` (`db_writer.py:229`) is the reference implementation: it takes **no `overwrite` parameter and issues no `DELETE` at all** — it inserts only loops whose `InMsec` does not already collide with an existing `Kind=0` row (skipped with a breadcrumb). Clobber-proof by construction and idempotent on re-run. **Never reuse `write_cues_to_db` to write loops** — it rewrites the memory *cue* set and cannot express a loop.
