...
- **Library Duplicates** (`autocue/analysis/duplicates.py` + `GET /api/duplicates` SSE + `POST /api/duplicates/delete`) groups tracks by case-insensitive whitespace-normalised `(artist, title)`, picks a keeper via `(play_count, existing_hot_cues, last_played, -track_id)`. Surfaces in a Library-tab panel with per-group + bulk delete. The delete path uses `db_writer.delete_tracks` (per-row savepoint + cascade through DjmdCue / DjmdSongHistory / DjmdSongPlaylist / DjmdSongTagList before the DjmdContent row), refuses with 409 when Rekordbox is running, and creates a timestamped backup before the first delete. The UI's confirm modal mirrors the Discover download-confirm pattern (primary disabled 250 ms after open, Cancel focused by default). Undo = `/api/restore` against the backup path the response returned. Tests at `tests/test_duplicates.py` (18 unit + 4 route specs) and `TestDuplicatesDeleteEndpoint` (5 specs).
